...
";
		...
		protected SsdpDevicePublisherBase(ISsdpCommunicationsServer communicationsServer, string osName, string osVersion, ISsdpLogger log)
		{
			...
			Log.LogInfo("Publisher started.");
			_CommsServer.BeginListeningForBroadcasts();
			Log.LogInfo("Publisher started listening for broadcasts.");
		}
		...
		/// </remarks>
		public ISsdpLogger Log { get; set; }
		...
		public bool SupportPnpRootDevice
		{
			...
			set
			{
				if (_SupportPnpRootDevice != value)
				{
					...
					Log.LogInfo("SupportPnpRootDevice set to " + value.ToString());
				}
			}
		}
		...
		public SsdpStandardsMode StandardsMode
		{
			...
			set
			{
				if (_StandardsMode != value)
				{
					...
					Log.LogInfo("StandardsMode set to " + value.ToString());
				}
			}
		}
		...
		public TimeSpan NotificationBroadcastInterval
		{
			...
			set
			{
				...
				if (_NotificationBroadcastInterval != value)
				{
					...
					Log.LogInfo("NotificationBroadcastInterval set to " + value.ToString());
		...
		protected override void Dispose(bool disposing)
		{
			if (disposing)
			{
				Log.LogInfo("Publisher disposed.");
		...
		private void ProcessSearchRequest(string mx, string searchTarget, UdpEndPoint endPoint)
		{
			...
			{
				Log.LogWarning(String.Format("Invalid search request received From {0}, Target is null/empty.", endPoint.ToString()));
				return;
			}

			Log.LogInfo(String.Format("Search Request Received From {0}, Target = {1}", endPoint.ToString(), searchTarget));
			...
			if (String.IsNullOrEmpty(mx))
			{
				...
				{
					Log.LogWarning("Search Request ignored due to missing MX header. Set StandardsMode to relaxed to respond to these requests.");
					return;
				}
			}
			...
			TaskEx.Delay(_Random.Next(16, (maxWaitInterval * 1000))).ContinueWith((parentTask) =>
			{
				...
				if (devices != null)
					...
					Log.LogWarning("Sending search responses for 0 devices (no matching targets).");
			});
		}
		...
		private IEnumerable<SsdpDevice> GetDevicesMatchingSearchTarget(string searchTarget, IEnumerable<SsdpDevice> devices)
		{
			lock (_Devices)
			{
				if (Devices.Any(x => x.NotificationType == searchTarget))
					devices = Devices.Where(x => x.NotificationType == searchTarget).ToArray();
				else if (String.Compare(SsdpConstants.SsdpDiscoverAllSTHeader, searchTarget,
					         StringComparison.OrdinalIgnoreCase) == 0)
					devices = GetAllDevicesAsFlatEnumerable().ToArray();
				else if (String.Compare(SsdpConstants.UpnpDeviceTypeRootDevice, searchTarget,
					         StringComparison.OrdinalIgnoreCase) == 0 ||
				         (IsWindowsExplorerSupportEnabled && String.Compare(SsdpConstants.PnpDeviceTypeRootDevice, searchTarget,
					          StringComparison.OrdinalIgnoreCase) == 0))
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		{
			Log.LogInfo(String.Format("Sending search (target = {1}) responses for {0} devices", devices.Count(), searchTarget));
		...
		private void SendDeviceSearchResponses(SsdpDevice device, string searchTarget, UdpEndPoint endPoint)
		{
			...
			if (isRootDevice && device.NotificationType == searchTarget)
			{
				SendSearchResponse(device.NotificationType, device, device.Usn ?? GetUsn(device.Udn, searchTarget), endPoint);
			}
			else if (isRootDevice && (sendAll || sendRootDevices))
			{
				SendSearchResponse(device.NotificationType ?? SsdpConstants.UpnpDeviceTypeRootDevice, device, device.Usn ?? GetUsn(device.Udn, SsdpConstants.UpnpDeviceTypeRootDevice), endPoint);
				if (IsWindowsExplorerSupportEnabled)
					SendSearchResponse(device.NotificationType ?? SsdpConstants.PnpDeviceTypeRootDevice, device, device.Usn ?? GetUsn(device.Udn, SsdpConstants.PnpDeviceTypeRootDevice), endPoint);
			}
		...
		private void SendAllAliveNotifications(object state)
		{
			try
			{
				...
				Log.LogInfo("Sending Alive Notifications For All Devices");
			...
			{
				Log.LogError("Publisher stopped, exception " + ex.Message);
				Dispose();
			}
			finally
			{
		...
		private void SendAliveNotifications(SsdpDevice device, bool isRoot)
		{
			if (isRoot)
			{
				SendAliveNotification(device, device.NotificationType ?? SsdpConstants.UpnpDeviceTypeRootDevice,  device.Usn ?? GetUsn(device.Udn, SsdpConstants.UpnpDeviceTypeRootDevice));
#pragma warning disable CS0618 // Type or member is obsolete
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		private void SetRebroadcastAliveNotificationsTimer(TimeSpan minCacheTime)
		{
			...
			Log.LogInfo(String.Format("Rebroadcast Interval = {0}, Next Broadcast At = {1}", rebroadCastInterval.ToString(), nextBroadcastInterval.ToString()));
		}
		...
		{
			Log.LogInfo(GetDeviceEventLogMessage(text, device));
		}

		private void LogDeviceEventWarning(string text, SsdpDevice device)
		{
			Log.LogWarning(GetDeviceEventLogMessage(text, device));
		}

		private void LogDeviceEventVerbose(string text, SsdpDevice device)
		{
			Log.LogVerbose(GetDeviceEventLogMessage(text, device));
		}
		...
		private void device_ServiceAdded(object sender, ServiceEventArgs e)
		{
			...
			Log.LogInfo(String.Format("Service added: {0} ({1})", e.Service.ServiceId, e.Service.FullServiceType));
		...
		{
			Log.LogInfo(String.Format("Service removed: {0} ({1})", e.Service.ServiceId, e.Service.FullServiceType));
		...
		private void CommsServer_RequestReceived(object sender, RequestReceivedEventArgs e)
		{
			...
			if (e.Message.Method.Method == SsdpConstants.MSearchMethod)
			{
				...
				if (!e.Message.Headers.Contains("MX") && !IsRelaxedStandardsMode)
					Log.LogWarning("Ignoring search request - missing MX header. Set StandardsMode to relaxed to process these search requests.");
				else if (!e.Message.Headers.Contains("MAN") && !IsRelaxedStandardsMode)
					Log.LogWarning("Ignoring search request - missing MAN header. Set StandardsMode to relaxed to process these search requests.");
				else
				...
				Log.LogWarning(String.Format("Unknown request \"{0}\"received, ignoring.", e.Message.Method.Method));
		}
